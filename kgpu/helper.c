...
static int kh_init(void) {
    ...
    hostvma.uva = (void*)mmap(NULL, KGPU_BUF_SIZE, PROT_READ | PROT_WRITE,
                              MAP_SHARED, devfd, 0);
    hostvma.size = KGPU_BUF_SIZE;
    if (hostvma.uva == MAP_FAILED) {
        kh_log(KGPU_LOG_ERROR, "set up mmap area failed\n");
        perror("mmap for GPU");
        abort();
    }
...
static struct _kgpu_sritem* kh_alloc_service_request() {
    struct _kgpu_sritem* s =
        (struct _kgpu_sritem*)malloc(sizeof(struct _kgpu_sritem));
    if (s) {
