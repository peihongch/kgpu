...
static int sync_callback(struct kgpu_request* req) {
    struct _kgpu_sync_call_data* data =
        (struct _kgpu_sync_call_data*)req->kdata;
...
static void kgpu_request_item_constructor(void* data) {
    struct _kgpu_request_item* item = (struct _kgpu_request_item*)data;
...
static void kgpu_sync_call_data_constructor(void* data) {
    struct _kgpu_sync_call_data* d = (struct _kgpu_sync_call_data*)data;
    if (d) {
...
struct kgpu_request* kgpu_alloc_request(void) {
    struct kgpu_request* req = kmem_cache_alloc(kgpu_request_cache, GFP_KERNEL);
    return req;
}
...
void kgpu_vfree(void* p) {
    unsigned long idx =
        ((unsigned long)(p) - (unsigned long)(kgpudev.gmpool.kva)) /
        KGPU_BUF_UNIT_SIZE;
    unsigned int nunits;
    ...
    if (nunits > (kgpudev.gmpool.nunits - idx)) {
        kgpu_log(KGPU_LOG_ERROR,
                 "incorrect GPU memory allocation info: "
...
unsigned long kgpu_alloc_mmap_area(unsigned long size) {
    ...
    idx = bitmap_find_next_zero_area(kgpudev.vm.bitmap, kgpudev.vm.npages, 0, n,
                                     0);
...
void kgpu_free_mmap_area(unsigned long start) {
    ...
    if (idx < 0 || idx >= kgpudev.vm.npages) {
        kgpu_log(KGPU_LOG_ERROR, "incorrect GPU mmap pointer 0x%lX to free\n",
                 start);
        return;
    }
...
void kgpu_unmap_area(unsigned long addr) {
    ...
    // return; // tmp workaround, this means we will consume the mem very
    // soon...
    ...
    if (idx < 0 || idx >= kgpudev.vm.npages) {
        ...
        if (n > 0) {
            ...
            if (ret)
                kgpu_log(KGPU_LOG_ALERT, "zap_vma_ptes returns %d\n", ret);
            kgpudev.vm.vma->vm_flags &= ~VM_PFNMAP;
            spin_unlock(&kgpudev.vm_lock);
        }
    }
...
static void fill_ku_request(struct kgpu_ku_request* kureq,
    ...
    if (ADDR_WITHIN(req->in, kgpudev.gmpool.kva,
        ...
        kureq->in =
            (void*)ADDR_REBASE(kgpudev.gmpool.uva, kgpudev.gmpool.kva, req->in);
    } else {
        kureq->in = req->in;
    }
    ...
    if (ADDR_WITHIN(req->udata, kgpudev.gmpool.kva,
                    kgpudev.gmpool.npages << PAGE_SHIFT)) {
        kureq->data = (void*)ADDR_REBASE(kgpudev.gmpool.uva, kgpudev.gmpool.kva,
                                         req->udata);
    } else {
...
ssize_t kgpu_read(struct file* filp, char __user* buf, size_t c, loff_t* fpos) {
    ...
    while (list_empty(&(kgpudev.reqs))) {
        ...
        if (wait_event_interruptible(kgpudev.reqq,
                                     (!list_empty(&(kgpudev.reqs)))))
            return -ERESTARTSYS;
        spin_lock(&(kgpudev.reqlock));
    }
...
ssize_t kgpu_write(struct file* filp,
            ...
            if (unlikely(kuresp.errcode != 0)) {
                switch (kuresp.errcode) {
                    ...
                    case KGPU_NO_SERVICE:
                        kgpu_log(KGPU_LOG_ALERT, "no such service %s\n",
                                 item->r->service_name);
                        break;
                    case KGPU_TERMINATED:
                        kgpu_log(KGPU_LOG_ALERT, "request is terminated\n");
                        break;
                    default:
                        kgpu_log(KGPU_LOG_ALERT, "unknown error with code %d\n",
                                 kuresp.id);
                        break;
                }
            }
...
static int set_gpu_mempool(char __user* buf) {
    ...
    if (!gmp->bitmap) {
        gmp->bitmap =
            kmalloc(BITS_TO_LONGS(gmp->nunits) * sizeof(long), GFP_KERNEL);
        if (!gmp->bitmap) {
    ...
    gmp->kva =
        (unsigned long)vmap(gmp->pages, gmp->npages, GFP_KERNEL, PAGE_KERNEL);
    if (!gmp->kva) {
...
static void set_vm(struct vm_area_struct* vma) {
    ...
    kgpudev.vm.alloc_sz = kmalloc(sizeof(u32) * kgpudev.vm.npages, GFP_KERNEL);
    kgpudev.vm.bitmap =
        kmalloc(BITS_TO_LONGS(kgpudev.vm.npages) * sizeof(long), GFP_KERNEL);
    if (!kgpudev.vm.alloc_sz || !kgpudev.vm.bitmap) {
        kgpu_log(KGPU_LOG_ERROR, "out of memory for vm's bitmap and records\n");
...
static int kgpu_mmap(struct file* filp, struct vm_area_struct* vma) {
    ...
    // SEE: https://blog.csdn.net/weixin_42096901/article/details/103228714
...
static int kgpu_init(void) {
    ...
    kgpu_request_cache =
        kmem_cache_create("kgpu_request_cache", sizeof(struct kgpu_request), 0,
