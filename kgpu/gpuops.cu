...
static int __merge_2ranges(unsigned long r1,
                           unsigned long s1,
                           unsigned long r2,
                           unsigned long s2,
                           unsigned long* e,
...
static int __merge_ranges(unsigned long ad[], unsigned long sz[], int n) {
    ...
    switch (n) {
        ...
        case 3:
            if (__merge_2ranges(ad[0], sz[0], ad[1], sz[1], &ad[0], &sz[0])) {
                if (__merge_2ranges(ad[0], sz[0], ad[2], sz[2], &ad[0],
                                    &sz[0])) {
                    return 1;
                } else {
                    ad[1] = ad[2];
            ...
            } else if (__merge_2ranges(ad[0], sz[0], ad[2], sz[2], &ad[0],
                                       &sz[0])) {
                if (__merge_2ranges(ad[0], sz[0], ad[1], sz[1], &ad[0],
                                    &sz[0])) {
                    return 1;
                } else
                    return 2;
            } else if (__merge_2ranges(ad[2], sz[2], ad[1], sz[1], &ad[1],
                                       &sz[1])) {
                if (__merge_2ranges(ad[0], sz[0], ad[1], sz[1], &ad[0], &sz[0]))
                    return 1;
                else
