...
static int crypt_iv_essiv_ctr(struct crypt_config* cc,
                              struct dm_target* ti,
                              const char* opts) {
    ...
    essiv_tfm = setup_essiv_cpu(cc, ti, salt, crypto_hash_digestsize(hash_tfm));
    if (IS_ERR(essiv_tfm)) {
...
static int crypt_iv_benbi_ctr(struct crypt_config* cc,
                              struct dm_target* ti,
                              const char* opts) {
...
static int crypt_iv_benbi_gen(struct crypt_config* cc,
                              u8* iv,
                              struct dm_crypt_request* dmreq) {
    __be64 val;
    ...
    val = cpu_to_be64(
        ((u64)dmreq->iv_sector << cc->iv_gen_private.benbi.shift) + 1);
...
static int crypt_iv_lmk_ctr(struct crypt_config* cc,
                            struct dm_target* ti,
                            const char* opts) {
    struct iv_lmk_private* lmk = &cc->iv_gen_private.lmk;
...
static int crypt_iv_lmk_one(struct crypt_config* cc,
                            u8* iv,
                            struct dm_crypt_request* dmreq,
                            u8* data) {
    struct iv_lmk_private* lmk = &cc->iv_gen_private.lmk;
    ...
    buf[1] =
        cpu_to_le32((((u64)dmreq->iv_sector >> 32) & 0x00FFFFFF) | 0x80000000);
...
static int crypt_iv_lmk_gen(struct crypt_config* cc,
                            u8* iv,
                            struct dm_crypt_request* dmreq) {
    u8* src;
    int r = 0;
...
static int crypt_iv_lmk_post(struct crypt_config* cc,
                             u8* iv,
                             struct dm_crypt_request* dmreq) {
    u8* dst;
    int r;
...
static int crypt_alloc_tfms(struct crypt_config* cc, char* ciphermode) {
    ...
    cc->tfms =
        kmalloc(cc->tfms_count * sizeof(struct crypto_ablkcipher*), GFP_KERNEL);
    if (!cc->tfms)
        return -ENOMEM;
...
static int crypt_setkey_allcpus(struct crypt_config* cc) {
    ...
    for (i = 0; i < cc->tfms_count; i++) {
        r = crypto_ablkcipher_setkey(cc->tfms[i], cc->key + (i * subkey_size),
                                     subkey_size);
        if (r)
            err = r;
    }
...
static int crypt_ctr_cipher(struct dm_target* ti, char* cipher_in, char* key) {
    ...
    ret =
        snprintf(cipher_api, CRYPTO_MAX_ALG_NAME, "%s(%s)", chainmode, cipher);
    if (ret < 0) {
        kfree(cipher_api);
        goto bad_mem;
    }
    ...
    if (cc->iv_size)
        ...
        cc->iv_size =
            max(cc->iv_size, (unsigned int)(sizeof(u64) / sizeof(u8)));
    else if (ivmode) {
...
static int crypt_map(struct dm_target* ti, struct bio* bio) {
    ...
    pr_info(
        "security_map: rw = %s, bio->bi_sector = %lu, bio->bi_size = %u, "
