...
static int setkey(struct crypto_tfm* parent,
                  const u8* key,
                  unsigned int keylen) {
    ...
    err = crypto_aes_expand_key(&ctx->aes_ctx, key, keylen / 2);
...
static int cpu_crypt(struct blkcipher_desc* d,
    ...
    struct sinfo s = {.tfm = crypto_cipher_tfm(ctx->child), .fn = fn};
    u8* wsrc;
    u8* wdst;
    sector_t tweak;
    ...
    for (;;) {
        ...
        do {
            // use sector num as tweak and one tweak per sector (512 bytes)
            if (unlikely(avail % 512 == 0)) {
                *((__le64*)(w->iv)) = cpu_to_le64(tweak++);
                *((__le64*)(w->iv + 8)) = cpu_to_le64(0);
...
static int gpu_crypt_zc(struct blkcipher_desc* d,
    ...
    addr = kgpu_alloc_mmap_area((inplace ? rsz : 2 * rsz) +
                                sizeof(struct crypto_xts_info));
    if (!addr) {
        free_page(TO_UL(data));
        g_log(KGPU_LOG_ERROR,
              "GPU buffer space is null for"
              "size %u inplace %d\n",
              rsz, inplace);
        return -ENOMEM;
    }
...
static int gpu_crypt(struct blkcipher_desc* desc,
                     struct scatterlist* dst,
                     struct scatterlist* src,
                     unsigned int nbytes,
                     int enc) {
    ...
    return zero_copy ? gpu_crypt_zc(desc, dst, src, ctx, nbytes, enc, NULL, 0)
                     : gpu_crypt_nzc(desc, dst, src, ctx, nbytes, enc, NULL, 0);
}

static int cpu_encrypt(struct blkcipher_desc* desc,
                       struct scatterlist* dst,
                       struct scatterlist* src,
                       unsigned int nbytes) {
    struct priv* ctx = crypto_blkcipher_ctx(desc->tfm);
...
static int cpu_decrypt(struct blkcipher_desc* desc,
                       struct scatterlist* dst,
                       struct scatterlist* src,
                       unsigned int nbytes) {
    struct priv* ctx = crypto_blkcipher_ctx(desc->tfm);
