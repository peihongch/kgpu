...
static void sha512_generic_block_fn(struct sha512_state* sst,
                                    u8 const* src,
                                    int blocks) {
    while (blocks--) {
...
int crypto_inc_hash_sha512_digest(struct shash_desc* desc,
                                  ...
                                  unsigned int len,
                                  u8* out) {
    const struct inc_hash_ctx* ctx = (const struct inc_hash_ctx*)data;
    int ret = 0;
    ...
    if (ctx->old_len && ctx->old_len == len) {
        ...
        ret = crypto_inc_hash_sha512_finish(desc, out);
    }
    ...
    sha512_base_do_update(desc, ctx->data + ctx->old_len, len,
                          sha512_generic_block_fn);
    sha512_base_do_finalize(desc, sha512_generic_block_fn);
    return crypto_inc_hash_sha512_finish(desc, out);
}

static struct shash_alg inc_hash_sha512_alg = {
    .digestsize = SHA512_DIGEST_SIZE,
    .digest = crypto_inc_hash_sha512_digest,
    .descsize = sizeof(struct sha512_state),
    .base = {
        .cra_name = "inc_hash-sha512",
        .cra_driver_name = "inc_hash-sha512-generic",
...
module_exit(inc_hash_sha512_mod_fini);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(
    "Incremental Hash Function built on SHA-512 Secure Hash Algorithms");

MODULE_ALIAS_CRYPTO("inc_hash-sha512");
MODULE_ALIAS_CRYPTO("inc_hash-sha512-generic");
